...
func TestRead(t *testing.T) { //nolint:funlen,gocognit,cyclop,maintidx
	...
	tests := []testroutines.Read{
		...
		{
			Name: "Read Opportunity with Contacts association via junction - OpportunityContactRoles subquery added and ContactIds extracted",
			Input: common.ReadParams{
				ObjectName:        "opportunity",
				Fields:            connectors.Fields("Name", "Amount", "StageName"),
			...
			Server: mockserver.Conditional{
				...
				If: mockcond.And{
					...
					mockcond.Or{
						...
						mockcond.QueryParam("q", "SELECT Name,Amount,StageName,(SELECT FIELDS(STANDARD) FROM OpportunityContactRoles) FROM opportunity"),
						mockcond.QueryParam("q", "SELECT (SELECT FIELDS(STANDARD) FROM OpportunityContactRoles),Name,Amount,StageName FROM opportunity"),
						mockcond.QueryParam("q", "SELECT Name,(SELECT FIELDS(STANDARD) FROM OpportunityContactRoles),Amount,StageName FROM opportunity"),
						mockcond.QueryParam("q", "SELECT Amount,Name,StageName,(SELECT FIELDS(STANDARD) FROM OpportunityContactRoles) FROM opportunity"),
						mockcond.QueryParam("q", "SELECT StageName,Name,Amount,(SELECT FIELDS(STANDARD) FROM OpportunityContactRoles) FROM opportunity"),
					},
				},
